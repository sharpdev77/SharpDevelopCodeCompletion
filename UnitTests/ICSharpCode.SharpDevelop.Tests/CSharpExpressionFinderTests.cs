...
class Main {
	...
	void Method() {
		...
		{
		    Assert.Inconclusive();
		    FindExpr(program2, "oundException", "NotF", ExpressionContext.TypeDerivingFrom(_projectContentRegistry.Mscorlib.SystemTypes.Exception, true));
			FindFull(program2, "oundException", "new NotFoundException()", ExpressionContext.TypeDerivingFrom(_projectContentRegistry.Mscorlib.SystemTypes.Exception, true));
		}
