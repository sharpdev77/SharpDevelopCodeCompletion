...
namespace ICSharpCode.AvalonEdit.CodeCompletion
{
    public class NRefactoryInsightWindowHandler : IInsightWindowHandler
    {
        ...
        public NRefactoryInsightWindowHandler(SupportedLanguage language, IProjectContent projectContent)
        {
            ...
            {
                eofToken = NRefactory.Parser.VB.Tokens.EOF;
                commaToken = NRefactory.Parser.VB.Tokens.Comma;
                openParensToken = NRefactory.Parser.VB.Tokens.OpenParenthesis;
                closeParensToken = NRefactory.Parser.VB.Tokens.CloseParenthesis;
                openBracketToken = -1;
                closeBracketToken = -1;
                openBracesToken = NRefactory.Parser.VB.Tokens.OpenCurlyBrace;
                closeBracesToken = NRefactory.Parser.VB.Tokens.CloseCurlyBrace;
                statementEndToken = NRefactory.Parser.VB.Tokens.EOL;
            ...
            if (r.Initialize(ParserService.GetParseInformation(editor.Document.Text, projectContent), cursorLocation.Y, cursorLocation.X))
            {
                ...
                if (currentMethod != null)
                {
                    ...
                    if (offset >= 0 && offset < editor.Document.TextLength)
                    {
                        ...
                        if (c == '(' || c == '[')
                        {
                            var insightProvider = new MethodInsightProvider(projectContent) { LookupOffset = offset };
        ...
        IInsightWindow ShowInsight(ITextEditor editor, IList<IInsightItem> insightItems, ICollection<ResolveResult> parameters, char charTyped)
        {
            ...
            else
            {
                ...
                IMethodOrProperty result = SharpDevelop.Dom.CSharp.OverloadResolution.FindOverload(methods.Where(m => m != null), argumentTypes, true, false, out overloadIsSure);
        ...
        bool ProvideContextCompletion(ITextEditor editor, IReturnType expected, char charTyped)
        {
            ...
            {
                CtrlSpaceCompletionItemProvider cdp = new NRefactoryCtrlSpaceCompletionItemProvider(languageProperties, _projectContent);
                var ctrlSpaceList = cdp.GenerateCompletionList(editor, _projectContent);
