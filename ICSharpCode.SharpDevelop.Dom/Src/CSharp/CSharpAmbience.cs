...
namespace ICSharpCode.SharpDevelop.Dom.CSharp
{
    ...
    {
        private static IDictionary<string, string> TypeConversionTable
        {
            get { return TypeReference.PrimitiveTypesCSharpReverse; }
        }

        private static bool ModifierIsSet(ModifierEnum modifier, ModifierEnum query)
        {
        ...
        public override string ConvertAccessibility(ModifierEnum accessibility)
        {
            if (ShowAccessibility)
            {
                ...
                if (ModifierIsSet(accessibility, ModifierEnum.Private))
                {
                    return "private ";
                }
                if (ModifierIsSet(accessibility, ModifierEnum.ProtectedAndInternal))
                {
                    return "protected internal ";
                }
                if (ModifierIsSet(accessibility, ModifierEnum.Internal))
                {
                    return "internal ";
                }
                if (ModifierIsSet(accessibility, ModifierEnum.Protected))
                {
                    return "protected ";
                }
            }
        ...
        private string GetModifier(IEntity decoration)
        {
            var ret = "";
            ...
            if (ShowModifiers)
            {
                ...
                if (c is CompoundClass)
                {
                    ...
                    builder.Append(string.Join(",",
                                               (c as CompoundClass).Parts.Select(p => p.CompilationUnit.FileName).
                                                   ToArray()));
                    builder.Append("} ");
                }
#endif
        ...
        private void AppendClassNameWithTypeParameters(StringBuilder builder, IClass c, bool fullyQualified,
                                                       bool isConvertingClassName, IList<IReturnType> typeArguments)
        {
            ...
            if (fullyQualified)
            {
                ...
                {
                    AppendClassNameWithTypeParameters(builder, c.DeclaringType, true, false, typeArguments);
                    builder.Append('.');
        ...
        public override string Convert(IMethod m)
        {
            ...
            if (IncludeBody)
            {
                ...
                {
                    builder.Append(m.DeclaringType.ClassType == ClassType.Interface ? ";" : " {");
                }
                else
                {
                    builder.Append(" {");
        ...
        private void AppendTypeNameForFullyQualifiedMemberName(StringBuilder builder, IReturnType declaringType)
        {
        ...
        private string ConvertTypeParameter(ITypeParameter tp)
        {
            return tp.BoundTo != null 
                ? Convert(tp.BoundTo)
                : tp.Name;
        }
        ...
        private static IReturnType GetElementType(IReturnType potentialArrayType)
        {
        ...
        private static void UnpackArrayType(StringBuilder builder, IReturnType returnType)
        {
