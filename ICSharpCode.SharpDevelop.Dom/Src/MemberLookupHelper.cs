...
//			}
        ...
        public static bool ConversionExists(IParameter from, IParameter to)
		{
			return ConversionExistsInternal(from.ReturnType, to.ReturnType, null);
		}
...
//				}
        ...
        public static bool SignatureMatch(IMethod delegateSignature, IMethod method)
        {
            if (method.Parameters.Count != delegateSignature.Parameters.Count)
                return false;
            if (!ConversionExists(method.ReturnType, delegateSignature.ReturnType))
                return false;
            if (Enumerable.Zip(delegateSignature.Parameters, method.Parameters, ConversionExists).Any(b => !b))
                return false;
            return true;
        }

        public static bool IsSpecialMember(IMethod method)
        {
            return method.Name == "Dispose" || method.Name == "Finalize" || method.Name == "#ctor";
        }

        public static bool IsAccesibleInCurrentContext(IMethod method, ResolveResult resolveResult)
        {
            var inStatic = resolveResult.CallingMember != null && resolveResult.CallingMember.IsStatic;
            
            if (inStatic && !method.IsStatic)
                return false;
            if (!method.IsAccessible(resolveResult.CallingClass, true))
                return false;
